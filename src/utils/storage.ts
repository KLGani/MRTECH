...
export const storage = {
  ...
  saveMark: (mark: StudentMark) => {
    const marks = storage.getAllMarks();
    marks.push(mark);
    localStorage.setItem(STORAGE_KEYS.MARKS, JSON.stringify(marks));
  },

  saveMultipleMarks: (marksArray: StudentMark[]) => {
    const allMarks = storage.getAllMarks();
  ...
  getAllMarks: (): StudentMark[] => {
    const data = localStorage.getItem(STORAGE_KEYS.MARKS);
    return data ? JSON.parse(data) : [];
  },

  getMarksByStudentEmail: (email: string): StudentMark[] => {
    return storage.getAllMarks().filter(m => m.studentEmail === email);
  },
